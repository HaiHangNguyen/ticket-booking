...
use Ticket\Directory\Model\ResourceModel\Ward\CollectionFactory as WardCollection;
use Ticket\Directory\Model\ResourceModel\City\CollectionFactory as CityCollection;
use Ticket\Directory\Model\ResourceModel\District\CollectionFactory as DistrictCollection;

/**
 * Class Data
 * @package Ticket\Directory\Helper
 */
class Data extends AbstractHelper
{
    /** @const - Cache Path */
    const CACHE_DIRECTORY_CITY_JSON_STORE = 'CACHE_DIRECTORY_CITY_JSON_STORE';
    const CACHE_TAG_CITY = 'CACHE_TAG_CITY';
    const CACHE_DIRECTORY_DISTRICT_JSON_STORE = 'CACHE_DIRECTORY_DISTRICT_JSON_STORE';
    const CACHE_TAG_DISTRICT = 'CACHE_TAG_DISTRICT';
    const CACHE_DIRECTORY_WARD_JSON_STORE = 'CACHE_DIRECTORY_WARD_JSON_STORE';
    const CACHE_TAG_WARD = 'CACHE_TAG_WARD';
    const CACHE_DIRECTORY_DATA_JSON_STORE = 'CACHE_DIRECTORY_DATA_JSON_STORE';
    const CACHE_TAG_DATA = 'CACHE_TAG_DATA';
    ...
    public function getCityOptions()
    {
        if (empty($this->_cityOptions)) {
            ...
            if (empty($json)) {
                $cities = $this->_cityCollection->create()->toOptionArray();
    ...
    public function getDistrictOptions()
    {
        if (empty($this->_districtOptions)) {
            ...
            if(empty($json)){
                $districts = $this->_districtCollection->create()->toOptionArray();
    ...
    public function getWardOptions()
    {
        if (empty($this->_wardOptions)) {
            ...
            if(empty($json)){
                $wards = $this->_wardCollection->create()->toOptionArray();
    ...
    public function getData()
    {
        ...
        foreach ($wards->prepareOptionArray() as $ward) {
        ...
        foreach ($districts->prepareOptionArray() as $district) {
        ...
        foreach ($cites->prepareOptionArray() as $city) {
